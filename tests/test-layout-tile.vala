...
void test_layout_create () {
  ...
  GLib.List<weak Gtk.Widget> children = layout.get_children ();
  assert (children.length () == 1);
  Gtk.HBox hbox = (Gtk.HBox) children.nth_data (0);
  assert (hbox is Gtk.HBox);
  GLib.List<weak Gtk.Widget> children_hbox = hbox.get_children ();
...
Gemini.Terminal get_zoom_from_layout (Gemini.Layout layout) {
  GLib.List<weak Gtk.Widget> children = get_hbox_from_layout (layout);
  assert (children.length () >= 1);
...
Gemini.Terminal get_terminal_from_stack (Gtk.VBox stack, int position) {
  GLib.List<weak Gtk.Widget> children = stack.get_children ();
